...
## 🗄️ CMDB 台帳基盤（Phase I-1 / Issue #287）

Excel 情報機器管理台帳を**単一の真実（正）**とした PC 資産マスタ。台帳 → CSV → DB の取り込みパイプラインを提供する（詳細: [`docs/CMDB_LEDGER.md`](docs/CMDB_LEDGER.md)）。

| 項目            | 内容                                                                                                                                                          |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🔑 識別キー     | 管理番号 = AD コンピュータ名 = `PC.pc_name`                                                                                                                   |
| 📒 マスタ       | `CMDB/■情報機器管理台帳.xlsx`（シート `管理番号ﾍﾞｰｽ`）                                                                                                        |
| 🧰 エクスポート | `python -m cmdb.export_cmdb_csv`（openpyxl・2019以降・Data-BackUp 自動退避・UTF-8 BOM）                                                                       |
| 📥 インポート   | `POST /api/cmdb/import`（admin）/ `python -m cmdb.import_cmdb`（dry-run 対応）                                                                                |
| 📊 状態         | `GET /api/cmdb/status`（台帳/agent/winrm の件数・最終同期時刻）                                                                                               |
| 🧬 拡張カラム   | `asset_number`/`owner_name`/`employee_id`/`deploy_year`/`mac_wired`/`mac_wireless`/`ip_lan`/`ip_wifi`/`ad_*`/`asset_source`/`ledger_synced_at`（全 nullable） |

```mermaid
flowchart LR
    XLSX["📒 情報機器管理台帳.xlsx"] -->|export_cmdb_csv| CSV["📄 CMDB.csv (UTF-8 BOM)"]
    CSV -.->|上書き前退避| BK["🗂️ Data-BackUp/"]
    CSV -->|"POST /api/cmdb/import"| DB[("🗃️ PC (asset_source=ledger)")]
...
**🖥️ PC-Ops Orchestrator** — _PC運用を、もっとスマートに。_
