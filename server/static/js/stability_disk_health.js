...
function renderPage() {
    ...
    const filtered = allItems.filter(it => {
        ...
        if (searchVal && !(it.pc_name || '').toLowerCase().includes(searchVal)) return false;
        return true;
    });
...
function exportCsv() {
    const rows = [['PC名', 'イベントID', 'ソース', 'ディスク', '重大度', 'メッセージ', '発生日時', '収集日時']];
    for (const it of allItems) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('severity-filter')?.addEventListener('change', () => { currentPage = 1; renderPage(); });
