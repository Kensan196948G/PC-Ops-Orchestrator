...
def upgrade():
    ...
    if not _table_exists("stability_scores"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["pc_id"], ["pcs.id"], name="fk_stability_scores_pc_id"
            ),
            sa.PrimaryKeyConstraint("id"),
        )
    if not _idx_exists("stability_scores", "ix_stability_scores_pc_id"):
        op.create_index(
            "ix_stability_scores_pc_id", "stability_scores", ["pc_id"], unique=False
        )
    if not _idx_exists("stability_scores", "ix_stability_scores_calculated_at"):
        op.create_index(
            "ix_stability_scores_calculated_at",
            "stability_scores",
            ["calculated_at"],
            unique=False,
        )
    ...
    if not _idx_exists("disk_health", "ix_disk_health_event_id"):
        op.create_index(
            "ix_disk_health_event_id", "disk_health", ["event_id"], unique=False
        )
    if not _idx_exists("disk_health", "ix_disk_health_generated_at"):
        op.create_index(
            "ix_disk_health_generated_at", "disk_health", ["generated_at"], unique=False
        )
    ...
    if not _idx_exists("known_issues", "ix_known_issues_kb_id"):
        op.create_index(
            "ix_known_issues_kb_id", "known_issues", ["kb_id"], unique=False
        )
    ...
    if not _idx_exists("event_logs", "ix_event_logs_category"):
        op.create_index(
            "ix_event_logs_category", "event_logs", ["category"], unique=False
        )
    if not _idx_exists("event_logs", "ix_event_logs_event_id"):
        op.create_index(
            "ix_event_logs_event_id", "event_logs", ["event_id"], unique=False
        )
    if not _idx_exists("event_logs", "ix_event_logs_generated_at"):
        op.create_index(
            "ix_event_logs_generated_at", "event_logs", ["generated_at"], unique=False
        )
    ...
    if not _col_exists("pcs", "last_stability_calc_at"):
        pcs_cols.append(
            sa.Column("last_stability_calc_at", sa.DateTime(), nullable=True)
        )
    if pcs_cols:
    ...
    if not _idx_exists("windows_updates", "ix_windows_updates_installed_at"):
        op.create_index(
            "ix_windows_updates_installed_at",
            "windows_updates",
            ["installed_at"],
            unique=False,
        )
    if not _idx_exists("windows_updates", "ix_windows_updates_kb_id"):
        op.create_index(
            "ix_windows_updates_kb_id", "windows_updates", ["kb_id"], unique=False
        )
