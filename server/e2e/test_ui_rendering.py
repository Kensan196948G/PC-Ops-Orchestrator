...
def test_404_page_status_and_content(page, live_server):
    ...
    response = page.goto(
        f"{live_server}/nonexistent-xyz-page", wait_until="domcontentloaded"
    )
...
def test_404_page_message(page, live_server):
    ...
    page.goto(f"{live_server}/this-does-not-exist", wait_until="domcontentloaded")
    body_text = page.text_content("body")
...
def test_alerts_page_renders(page_with_login, live_server):
    ...
    errors = []
    p.on("pageerror", lambda e: errors.append(str(e)))
    p.goto(f"{live_server}/alerts", wait_until="domcontentloaded")
    p.wait_for_load_state("networkidle", timeout=10000)
