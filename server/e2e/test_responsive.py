...
def test_tablet_layout_has_sidebar_element(page_with_login, live_server):
    ...
    assert p.locator(".sidebar").count() > 0, "Sidebar element must exist at tablet width"
...
def test_mobile_layout_has_sidebar_element(page_with_login, live_server):
    ...
    p.goto(f"{live_server}/")
    p.wait_for_load_state("domcontentloaded", timeout=8000)
    assert p.locator(".sidebar").count() > 0, "Sidebar element must exist at mobile width"
...
def test_desktop_main_content_visible(page_with_login, live_server):
    ...
    p.goto(f"{live_server}/")
    p.wait_for_load_state("domcontentloaded", timeout=8000)
    assert p.locator(".main-content").is_visible(), ".main-content must be visible on desktop"
