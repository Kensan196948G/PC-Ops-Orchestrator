...
def collect():
    ...
    pc.cpu_logical_processors = data.get('cpu_logical_processors', pc.cpu_logical_processors)
    ...
    mem_usage = 0
    if pc.memory_total_gb and pc.memory_total_gb > 0 and pc.memory_available_gb is not None:
        mem_usage = (pc.memory_total_gb - pc.memory_available_gb) / pc.memory_total_gb * 100
    pc.health_score = _calculate_health_score(pc)

    disk_usage = 0
    if pc.disk_total_gb and pc.disk_total_gb > 0 and pc.disk_free_gb is not None:
...
def _get_pending_tasks(pc_id):
    ...
    tasks = Task.query.filter(
        (Task.pc_id == pc_id) | (Task.pc_id.is_(None)),
        Task.status == 'pending'
